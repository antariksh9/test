	...
	var BaseView = Backbone.View,
		...
		AddTaskFormView = BaseView.extend({
			...
			render: function() {
				...
				$(div)
					.find("#overlay-initial-task div#form-head div#cross")
					.on("click", window.functions.addEventsButtons);
				$(div)
					.find("#overlay-initial-task div#form-foot a#cancel-project")
					.on("click", window.functions.addEventsButtons);
				$(div)
					.find("#overlay-initial-task div#form-foot a#create-new-project")
					.on("click", submitTaskFormData);
				$el.append($(div)
					.find('#overlay-initial-task'));
				return _this;
			}
