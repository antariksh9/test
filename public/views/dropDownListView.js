	...
	var BaseView = Backbone.View,
		...
		DropDownListView = BaseView.extend({
			...
			render: function () {
				...
				$el.append(Mustache.render(_this.template, {class:data.task.id}));
				
				list=$el.find('#dropdown-status');

				right=data.parent.offsetLeft+$(data.parent).width();
				top = data.parent.offsetTop+$(data.parent).height() - 3;
				left=right-$(list).width(); 
				if(left<0)
				{
					left=0;
				}
